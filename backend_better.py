...
import threading
import time
from typing import List, Literal, Optional, AsyncGenerator, Dict
...
@app.get("/health")
async def health():
    return {"ok": True, "accounts": len(_accounts), "model": DEFAULT_MODEL}

@app.get("/accounts")
def list_accounts():
    return [
        {"name": account.name, "number": account.number, "amount": account.amount}
...
def create_account(payload: AccountCreate):
    ...
    return {"name": account.name, "number": account.number, "amount": account.amount}

@app.post("/accounts/{number}/deposit")
def deposit(number: int, payload: AmountRequest):
    try:
        account = _get_account(number)
        _deposit(account, payload.amount)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    return {"number": account.number, "amount": account.amount}

@app.post("/accounts/{number}/withdraw")
def withdraw(number: int, payload: AmountRequest):
    try:
        account = _get_account(number)
        _withdraw(account, payload.amount)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
